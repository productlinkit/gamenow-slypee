...
const FILTERS = [["all", "All"], ["app", "Games"], ["h5", "HTML5"]];
  ...
  return (
    <li className="srow" style={{ "--i": Math.min(i, 12) }}>
      <a className="srow-main" href={detailHref(g)} onClick={onPick}>
        ...
        <span className="txt">
          <b>{highlight(g.n, q).map(([t, m], k) => m ? <mark key={k}>{t}</mark> : t)}</b>
          <small>
            <span className="star">★ {g.r.toFixed(1)}</span>
            <span>{g.c[0]}</span>
            <span className="plays" title={`${fmt(g.p)} plays`}>{short(g.p)} plays</span>
            {g.t === "h5" && <span className="badge">HTML5</span>}
          </small>
        </span>
      </a>
      <a className="btn-play" href={detailHref(g)} onClick={onPick}><PlayIcon />Play</a>
    </li>
  );
}
...
export default function SearchSheet({ initialType = "all", onClose }) {
  ...
  return (
    <>
      ...
      <div className="sheet" role="dialog" aria-modal="true" aria-label="Search games">
        <div className="sheet-bar">
          ...
          <form className="search" role="search" onSubmit={e => { e.preventDefault(); remember(q); inputRef.current?.blur(); }}>
            ...
            <input
              ...
              placeholder={`Search ${GAMES.length} games…`} aria-label="Search games"
              value={q} onChange={e => setQ(e.target.value)}
            />
            {q && <button type="button" className="clear-btn" aria-label="Clear search" onClick={() => { setQ(""); inputRef.current?.focus(); }}>✕</button>}
          </form>
        </div>
        ...
        <div className="sheet-body" ref={bodyRef}>
          {!term ? (
            <>
              {recent.length > 0 && (
                <section className="sheet-sec">
                  <h3>Recent <button type="button" className="link-btn" onClick={clearRecent}>Clear</button></h3>
                  <ul className="recent">
                    {recent.map(r => (
                      <li key={r}>
                        ...
                        <button type="button" className="recent-x" aria-label={`Remove ${r}`} onClick={() => forget(r)}>✕</button>
                      </li>
                    ))}
                  </ul>
                </section>
              ...
              <section className="sheet-sec">
                ...
                <div className="chips">{CATS.map(c => <button key={c} type="button" className="chip" onClick={() => run(c)}>{CAT_ICONS[c]}{c}</button>)}</div>
              </section>
              <section className="sheet-sec">
                <h3>Trending now</h3>
            ...
            <>
              ...
              {hits.length ? (
                <section className="sheet-sec">
                  <p className="sheet-count" aria-live="polite">{hits.length} {hits.length === 1 ? "game" : "games"} for “{term}”</p>
                ...
                <section className="sheet-sec no-hits" aria-live="polite">
                  ...
                  <h3>No games for “{term}”{type !== "all" && all.length ? ` in ${type === "h5" ? "HTML5" : "Games"}` : ""}</h3>
                  {type !== "all" && all.length
                    ? <button type="button" className="btn-play" onClick={() => setType("all")}>Show all {all.length} results</button>
                    : <><p>Check the spelling or try one of these:</p><div className="chips">{POPULAR.slice(0, 5).map(p => <button key={p} type="button" className="chip" onClick={() => run(p)}>{p}</button>)}</div></>}
                </section>
              )}
            </>
          )}
